...
export function LinkBioListPage() {
  ...
  useEffect(() => {
    api
      .get<Product[]>('/products/', { params: { product_type: 'link_in_bio' } })
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-heading">Link-in-Bio Pages</h1>
          <p className="text-sm text-muted mt-1">Create and manage your link-in-bio pages</p>
        </div>
        <button
      ...
      {products.length === 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
          ...
          <h2 className="text-xl font-bold text-heading mb-2">No link-in-bio pages yet</h2>
          <p className="text-sm text-muted mb-6">Create your first link-in-bio page and share all your links in one place.</p>
          <button
            onClick={() => navigate('new')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {products.map((product, i) => {
          ...
          const viewCount = (product.config_data?.view_count as number) ?? 0;
          const displayName = (product.config_data?.display_name as string) || product.name;
            ...
            >
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-bold text-heading truncate">{displayName}</h3>
